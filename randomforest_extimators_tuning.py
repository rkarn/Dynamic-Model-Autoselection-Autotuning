...
estimators_number = list(range(10,105,5))

# empty list that will hold cv scores
cv_scores = []

# perform 10-fold cross validation
for k in estimators_number:
    clf = RandomForestClassifier(n_estimators=k)
    print "Creating the model for ",k," estimators."
    clf.fit(X_train, Y_train)
    print "Testing the model for ",k," estimators."
    pred = clf.predict(X_test)
    print "Calculating accuracy score for ",k," estimators."
    scores = round(accuracy_score(Y_test, pred),3)*100
    cv_scores.append(scores)

print "Number of estimators:",estimators_number
