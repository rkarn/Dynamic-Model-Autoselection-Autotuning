...
while (dataspace < len(X_test)):
    for clf in model_list:
        pred = clf.predict(X_test[dataspace,:].reshape(1, -1))
        print "machine learning =",str(clf).split("(")[0], "Actual={0}    Predicted={1}".format(Y_test[dataspace],pred)
    dataspace = dataspace+1000
    time.sleep(1)
    print "-----------*----------------"
